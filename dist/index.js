...
let raod_4_CPM = 1;
class Road {
    constructor(roadName, carPerMinute) {
        this.roadName = roadName;
        this.carPerMinute = carPerMinute;
    }
    setCarPerMinute(carPerMinute) {
...
class Intersection {
    constructor(intersectionName, controlProperty, efficient_HighTroughput, // Total CPM >= 20
    efficient_MediumTroughput, // Total 20 > CPM >= 10
    efficient_LowTroughput) {
        this.intersectionName = intersectionName;
        this.controlProperty = controlProperty;
        this.efficient_HighTroughput = efficient_HighTroughput;
        this.efficient_MediumTroughput = efficient_MediumTroughput;
        this.efficient_LowTroughput = efficient_LowTroughput;
    }
}
const Intersection1 = new Intersection('Roundabout', 'Free flowing at low CPM, but at higher CPMs, there is extra congestion and inefficiency.', 50, 75, 90);
const Intersection2 = new Intersection('Stop Signs', 'No waiting if the intersection is clear, but all users must stop, so it is inefficient at low CPM.', 20, 30, 40);
const Intersection3 = new Intersection('Traffic Lights', 'Efficient at high CPM, but users may have to wait, even at low CPM', 90, 75, 30);
...
const dispInputs = roads.map(road => `CPM of ${road.roadName} is set to be ${road.getCarPerMinute()}`);
dispInputs.map(dispInput => console.log(dispInput));
// array contains all the intersections
const intersections = [Intersection1, Intersection2, Intersection3];
// calculate total CPM
const totalCPM = (roads) => {
    return roads.reduce((a, b) => a + b.getCarPerMinute(), 0); //sum of CPM of all roads in the array
};
// console.log(totalCPM(roads));
// console.log(intersections);
// calculateEfficiencyScore
const calEfficiencyScores = (intersections, cpm) => {
    let efficiency;
    if (cpm >= 20) {
...
const dispCalEfficiencyScores = (intersections, cpm) => {
    let efficiency;
    if (cpm >= 20) {
        efficiency = intersections.efficient_HighTroughput;
    }
...
// console.log(formEl);
const inputRoad1CPM_El = document.querySelector('#Road1CPM');
// console.log(inputRoad1CPM_El);
const inputRoad2CPM_El = document.querySelector('#Road2CPM');
// console.log(inputRoad2CPM_El);
const inputRoad3CPM_El = document.querySelector('#Road3CPM');
// console.log(inputRoad3CPM_El);
const inputRoad4CPM_El = document.querySelector('#Road4CPM');
